...
PerceptionObstacle::Type Obstacle::type() const { return type_; }  //type

bool Obstacle::IsPedestrian() const {
  return type_ == PerceptionObstacle::PEDESTRIAN;
}  //IsPedestrian

int Obstacle::id() const { return id_; }  //id
...
Feature* Obstacle::mutable_latest_feature() {
  ACHECK(!feature_history_.empty());
  return &(feature_history_.front());
}//mutable_lastest_feature

size_t Obstacle::history_size() const { return feature_history_.size(); } //history_size

bool Obstacle::IsStill() {
  if (feature_history_.size() > 0) {
    return feature_history_.front().is_still();
  }
  return true; //bool,是否为变
}
...
bool Obstacle::Insert(const PerceptionObstacle& perception_obstacle,
  ...
  SetStatus(perception_obstacle, timestamp, &feature);//status

  // Set obstacle lane features
  if (type_ != PerceptionObstacle::PEDESTRIAN) {
    SetCurrentLanes(&feature);  //currentLanes
    SetNearbyLanes(&feature);  //nearbylanes
  }
...
void Obstacle::SetStatus(const PerceptionObstacle& perception_obstacle,
  ...
  SetPolygonPoints(perception_obstacle, feature); //polygonPoints 多线段点
  SetPosition(perception_obstacle, feature);   //位置
  SetVelocity(perception_obstacle, feature);   //速度
  SetAcceleration(feature);                    //加速度
  SetTheta(perception_obstacle, feature);      //theta
  SetLengthWidthHeight(perception_obstacle, feature);//长宽高
  SetIsNearJunction(perception_obstacle, feature);   //是否是最近的交叉口
}
...
void Obstacle::SetCurrentLanes(Feature* feature) {
  Eigen::Vector2d point(feature->position().x(), feature->position().y()); //vector2d
  double heading = feature->velocity_heading();  //heading
  int max_num_lane = FLAGS_max_num_current_lane; //max_num
  double max_angle_diff = FLAGS_max_lane_angle_diff; //max_angle_diff
  double lane_search_radius = FLAGS_lane_search_radius; //lane_search_radius
  ...
  std::vector<std::shared_ptr<const LaneInfo>> current_lanes; //current_lanes
  ...
  for (std::shared_ptr<const LaneInfo> current_lane : current_lanes) {
    ...
    LaneFeature* lane_feature = lane.add_current_lane_feature();/add_current_lane_feature
    lane_feature->set_lane_turn_type(turn_type);  //turn_type
    lane_feature->set_lane_id(lane_id);           //lane_id
    lane_feature->set_lane_s(s);                  //s
    lane_feature->set_lane_l(l);                  //l
    lane_feature->set_angle_diff(angle_diff);     //angle_diff
    lane_feature->set_lane_heading(nearest_point_heading); //lane_heading
    lane_feature->set_dist_to_left_boundary(left - l);     //dist_to_left_boundary
    lane_feature->set_dist_to_right_boundary(right + l);   //dist_to_right_boundary
    lane_feature->set_lane_type(current_lane->lane().type()); //lane_type
