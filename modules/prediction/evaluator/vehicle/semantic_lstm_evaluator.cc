...
//semantic_map可以在给定obstacle的当前central_pos来获取周边的的道路,车道(中心线,左边界,右边界),交叉口,人行横道,等
SemanticLSTMEvaluator::SemanticLSTMEvaluator(SemanticMap* semantic_map) // 传入地图,然后加载模型,
...
bool SemanticLSTMEvaluator::Evaluate(Obstacle* obstacle_ptr,
  ...
  obstacle_ptr->SetEvaluatorType(evaluator_type_); //评估的类型:行人,车辆

  Clear();
  CHECK_NOTNULL(obstacle_ptr);
  int id = obstacle_ptr->id(); //id
  if (!obstacle_ptr->latest_feature().IsInitialized()) {
    AERROR << "Obstacle [" << id << "] has no latest feature.";
    return false;
  }
  Feature* latest_feature_ptr = obstacle_ptr->mutable_latest_feature(); //get the lastest_feature ptr
  ...
  // Process the feature_map    change BGR to RGB, cv::COLOR_BGR2RGB
  cv::cvtColor(feature_map, feature_map, cv::COLOR_BGR2RGB); // feature_map1 to feature_map2
  cv::Mat img_float;
  feature_map.convertTo(img_float, CV_32F, 1.0 / 255);
  torch::Tensor img_tensor = torch::from_blob(img_float.data, {1, 224, 224, 3});
  img_tensor = img_tensor.permute({0, 3, 1, 2});  //b,c,l,w
  img_tensor[0][0] = img_tensor[0][0].sub(0.485).div(0.229);
  img_tensor[0][1] = img_tensor[0][1].sub(0.456).div(0.224);
  img_tensor[0][2] = img_tensor[0][2].sub(0.406).div(0.225);

  // Extract features of pos_history
  std::vector<std::pair<double, double>> pos_history(20, {0.0, 0.0});//pos的过去20帧的(x,y)
  ...
  std::vector<torch::jit::IValue> torch_inputs; //torch_inputs

  torch_inputs.push_back(c10::ivalue::Tuple::create(
      {std::move(img_tensor.to(device_)), std::move(obstacle_pos.to(device_)),
       std::move(obstacle_pos_step.to(device_))})); //img_tensor, obstacle_pos, obstacle_pos_step
  ...
  if (obstacle_ptr->IsPedestrian()) {
    torch_output_tensor = torch_pedestrian_model_.forward(torch_inputs)
                              .toTensor()
                              .to(torch::kCPU);
  } else {   //vehicle_tensor_model
    torch_output_tensor =
        torch_vehicle_model_.forward(torch_inputs).toTensor().to(torch::kCPU);  //torch_output_tensor
  }

  auto end_time = std::chrono::system_clock::now();
  std::chrono::duration<double> diff = end_time - start_time;  //diff
  ADEBUG << "Semantic_LSTM_evaluator used time: " << diff.count() * 1000
         << " ms.";
  auto torch_output = torch_output_tensor.accessor<float, 3>();//torch_output

  // Get the trajectory   (pos_x, pos_y)
  double pos_x = latest_feature_ptr->position().x();
  double pos_y = latest_feature_ptr->position().y();
  Trajectory* trajectory = latest_feature_ptr->add_predicted_trajectory(); //trajectory
