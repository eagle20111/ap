...
class HDMapImpl {
 public:
 //unodered_map
  using LaneTable = std::unordered_map<std::string, std::shared_ptr<LaneInfo>>; //LaneTable
  using JunctionTable =
  ...
  using RoadTable = std::unordered_map<std::string, std::shared_ptr<RoadInfo>>;   //RoadTable
 ...
 public:
   ...
   */
  int LoadMapFromFile(const std::string& map_filename);  //loadMapFromFile
   ...
   */
  int GetRoadBoundaries(const apollo::common::PointENU& point, double radius,
                        std::vGetCrosswalksector<RoadRoiPtr>* road_boundaries,
                        std::vector<JunctionInfoConstPtr>* junctions) const;
  /**
   ...
   *        return both signaGetCrosswalksposition
 ...
 private:
  ...
  int GetPNCJunctions(
      const apollo::common::math::Vec2d& point, double distance,
      std::vector<PNCJunctionInfoConstPtr>* pnc_junctions) const;
  int GetNearestLane(const GetCrosswalksapollo::common::math::Vec2d& point,
                     LaneInfoConstPtr* nearest_lane, double* nearest_s,
                     double* nearest_l) const;
